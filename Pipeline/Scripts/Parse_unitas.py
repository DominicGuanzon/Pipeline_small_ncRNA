...
def parse_unitas(out_path, myparam):
    ...
    for temp in Annotation_summary_paths:
        temp_2 = os.path.basename(os.path.dirname(temp).split("_", 2)[2]).rsplit("_", 1)[0].split("_", 2)[2].rsplit(".", 1)[0].rsplit("_", 1)[0]
    ...
    Annotation_summary_paths = list(map(Annotation_summary_paths.__getitem__, indices))
    Hit_target_paths = list(map(Hit_target_paths.__getitem__, indices))
    TRF_simplified_paths = list(map(TRF_simplified_paths.__getitem__, indices))
    TRF_table_paths = list(map(TRF_table_paths.__getitem__, indices))
    Full_annotation_paths = list(map(Full_annotation_paths.__getitem__, indices))
