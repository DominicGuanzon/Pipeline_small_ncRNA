...
def parse_unitas(out_path, myparam):
    ...
    miRNA_paths = glob.glob(myparam + "*/miR-table_*", recursive=True)
    ...
    miRNA_paths = list(map(miRNA_paths.__getitem__, rearrange_paths_indices(miRNA_paths, Sample_file)))
    ...
    # Merge miRNA_paths output files
    data = []
    
    for csv in miRNA_paths:
        frame = pd.read_csv(csv, sep = "\t")
    ...
    df_merged = reduce(lambda  left,right: pd.merge(left,right,on=["miR_name"], how="outer"), data)
    df_merged.to_csv(out_path[5], index = False, na_rep = "NA")
